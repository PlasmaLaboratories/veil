...
def Module.defineTransitionAbstractForNext (mod : Module) : TermElabM (Option Command) := do
  ...
  -- Prepare
  let [r₀, s₀, s₁, idxs] := [`r₀, `s₀, `s₁, `idxs].map mkVeilImplementationDetailIdent | unreachable!
  let sortIdents ← mod.uninterpretedParamIdents
  let abstractStateSortTerm ← `($fieldAbstractDispatcher $sortIdents*)
  let abstractStateTypeTerm ← `($stateIdent $abstractStateSortTerm)
  let theoryTy ← `($theoryIdent $sortIdents*)
  ...
  let res ← actions.mapM fun s => do
    ...
    let rhs ← mkFunSyntax actualBinders <| ← `(term| @$tr $specializedArgs* $absr₀ $absst₀ $absst₁)
    let placeholders := Array.replicate actualParams.size hole
    let trPair ← `(⟨[$placeholders,*], ($lhs, $rhs)⟩)
    let weakeningThm ←
      `(@$(mkIdent <| toTransitionAbstractName <| toExtName s.name) $allArgs* $r₀ $s₀ $s₁)
    let h := mkVeilImplementationDetailIdent `h
    let proof ← `(by
      have $h := $weakeningThm
      __veil_neutralize_decidable_inst ! at $h ⊢
      exact $h)
    let thm ← mkFunSyntax actualBinders proof
    pure (trPair, thm)
  ...
  let body ←
    `(by
      classical
      exact $(mkIdent ``IteratedPred.bigor_exists_implies) [$actionTrPairs,*] $bigAndProof $idxs)
  `(def $(mkIdent <| toTransitionAbstractName assembledNextName) $allBinders* := $body)
